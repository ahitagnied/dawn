...
const applyPhraseReplacements = (text: string, phraseReplacements: PhrasePair[]): string => {
  ...
  phraseReplacements.forEach(phrase => {
    ...
    console.log('Applying replacement:', { original: phrase.original, replacement: phrase.replacement, regex: regex.toString() })
    const before = result
...
const normalizeTranscription = (raw: RawTranscription, phraseReplacements: PhrasePair[] = []): Transcription => {
...
export function useTranscriptions(phraseReplacements: PhrasePair[] = []): { transcriptions: Transcription[] } {
  ...
  useEffect(() => {
    const unsubscribe = window.bridge?.onTranscriptionAdd?.((transcription) => {
      setTranscriptions((prev) => [normalizeTranscription(transcription, phraseReplacements), ...prev])
    })
    return unsubscribe
  }, [phraseReplacements])
